...
from .render_tools import export_emit_image, export_visibility, export_canny, export_tile, bake_texture, prepare_baking, unwrap # pylint: disable=relative-beyond-top-level
...
class ComfyUIGenerate(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if context.scene.generation_method == 'sequential' and context.scene.sequential_custom_camera_order != "":
            ...
            raw_order = context.scene.sequential_custom_camera_order.split(',')

            try:
                camera_order = [int(index.strip()) for index in raw_order]
            except ValueError:
                self.report({'ERROR'}, "Invalid input: all values must be integer indices.")
                context.scene.generation_status = 'idle'
                ComfyUIGenerate._is_running = False
                return {'CANCELLED'}

            # Check if number of indices matches number of cameras
            ...
            if any(index < 0 or index >= len(self._cameras) for index in camera_order):
                self.report({'ERROR'}, "One or more indices are out of valid camera range.")
                context.scene.generation_status = 'idle'
                ComfyUIGenerate._is_running = False
                return {'CANCELLED'}

            # Valid input: proceed with reorder
    ...
    def modal(self, context, event):
        ...
        if event.type == 'TIMER':
            ...
            if not self._thread.is_alive():
                ...
                if self._error:
                    if self._error == "'25'" or self._error == "'111'" or self._error == "'32'":
            ...
            while self._threads_left > 0 and not context.scene.project_only:
                if context.scene.steps != 0:
                    ...
                    if context.scene.generation_method != 'uv_inpaint':
                        ...
                        tile_path = get_file_path(context, "controlnet", subtype="tile", camera_id=camera_id) if camera_id is not None else get_file_path(context, "controlnet", subtype="tile")
                    else:
                    ...
                    if context.scene.generation_method == 'refine':
                        ...
                        if context.scene.model_architecture == 'flux1':
                            image = self.refine_flux(context, depth_path=depth_path, canny_path=canny_path, normal_path=normal_path, tile_path=tile_path, render_path=render_path)
                        elif context.scene.model_architecture == 'chroma':
                            image = self.refine_chroma(context, depth_path=depth_path, canny_path=canny_path, normal_path=normal_path, tile_path=tile_path, render_path=render_path)
                        else:
                            image = self.refine(context, depth_path=depth_path, canny_path=canny_path, normal_path=normal_path, tile_path=tile_path, render_path=render_path)

                    elif context.scene.generation_method == 'uv_inpaint':
                        if context.scene.model_architecture == 'flux1':
                            image = self.refine_flux(context, mask_path=mask_path, render_path=render_path)
                        elif context.scene.model_architecture == 'chroma':
                            image = self.refine_chroma(context, mask_path=mask_path, render_path=render_path)
                        else:
                        ...
                        if self._current_image == 0:
                            if context.scene.model_architecture == 'flux1':
                                image = self.generate_flux(context, depth_path=depth_path, canny_path=canny_path, normal_path=normal_path, tile_path=tile_path)
                            elif context.scene.model_architecture == 'chroma':
                                image = self.generate_chroma(context, depth_path=depth_path, canny_path=canny_path, normal_path=normal_path, tile_path=tile_path)
                            else:
                                image = self.generate(context, depth_path=depth_path, canny_path=canny_path, normal_path=normal_path, tile_path=tile_path)
                        else:
                            ...
                            if context.scene.model_architecture == 'flux1':
                                image = self.refine_flux(context, depth_path=depth_path, render_path=render_path, mask_path=mask_path, canny_path=canny_path, normal_path=normal_path, tile_path=tile_path)
                            elif context.scene.model_architecture == 'chroma':
                                image = self.refine_chroma(context, depth_path=depth_path, render_path=render_path, mask_path=mask_path, canny_path=canny_path, normal_path=normal_path, tile_path=tile_path)
                            else:
                                image = self.refine(context, depth_path=depth_path, render_path=render_path, mask_path=mask_path, canny_path=canny_path, normal_path=normal_path, tile_path=tile_path)
                    ...
                    if context.scene.generation_method == 'sequential':
                        ...
                        if self._current_image < len(self._cameras) - 1:
                            ...
                            tile_path = get_file_path(context, "controlnet", subtype="tile", camera_id=next_camera_id)
                ...
                if context.scene.generation_method == 'separate' or context.scene.generation_method == 'refine' or context.scene.generation_method == 'sequential':
                    ...
                    if context.scene.refine_images:
                        for i, _ in enumerate(self._cameras):
                            ...
                            refine_tile_path =  get_file_path(context, "controlnet", subtype="tile", camera_id=i)
                            ...
                            if context.scene.model_architecture == 'flux1':
                                image = self.refine_flux(context, depth_path=refine_depth_path, canny_path=refine_canny_path, normal_path=refine_normal_path, tile_path=refine_tile_path, render_path=refine_render_path)
                            elif context.scene.model_architecture == 'chroma':
                                image = self.refine_flux(context, depth_path=refine_depth_path, canny_path=refine_canny_path, normal_path=refine_normal_path, tile_path=refine_tile_path, render_path=refine_render_path)
                            else:
                                image = self.refine(context, depth_path=refine_depth_path, canny_path=refine_canny_path, normal_path=refine_normal_path, tile_path=refine_tile_path, render_path=refine_render_path)
    ...
    def generate(self, context, depth_path=None, canny_path=None, normal_path=None, tile_path=None):
        """     
        ...
        :param normal_path: Path to the tile map image.
        ...
        prompt = self._build_controlnet_chain(prompt, context, depth_path, canny_path, normal_path, tile_path, NODES)
    ...
    def _build_controlnet_chain_extended(self, context, base_prompt, pos_input, neg_input, vae_input, image_dict):
        ...
        # Mapping is deprecated and replaced by direct ControlNet selection
        addon_prefs = context.preferences.addons[__package__].preferences
        try:
        ...
        for idx, unit in enumerate(controlnet_units):
            ...
            if unit.is_union and unit.use_union_type: 
                ...
                elif unit.unit_type == "tile":
                    base_prompt["239"]["inputs"]["type"] = "tile"
                has_union = True
        if not has_union:
            # Remove the node
    ...
    def _build_controlnet_chain(self, prompt, context, depth_path, canny_path, normal_path, tile_path, NODES):
        ...
        prompt, final_node = self._build_controlnet_chain_extended(
            ...
            {"depth": depth_path, "canny": canny_path, "normal": normal_path, "tile": tile_path}
        )
    ...
    def refine(self, context, depth_path=None, canny_path=None, normal_path=None, tile_path=None, mask_path=None, render_path=None):
        """     
        ...
        :param normal_path: Path to the normal map image.
        :param tile_path: Path to the tile map image.
        ...
        self._refine_configure_images(prompt, depth_path, canny_path, normal_path, tile_path, render_path, NODES)
        
        # Build controlnet chain for refinement if needed
        if not context.scene.generation_method == 'uv_inpaint':
            prompt = self._refine_build_controlnet_chain(prompt, context, depth_path, canny_path, normal_path, tile_path, NODES)
        else:
    ...
    def _refine_configure_images(self, prompt, depth_path, canny_path, normal_path, tile_path, render_path, NODES):
    ...
    def _refine_build_controlnet_chain(self, prompt, context, depth_path, canny_path, normal_path, tile_path, NODES):
    ...
    def combine_maps(self, context, cameras, type):
        ...
        if type == 'depth':
            ...
            grid_image_path = get_file_path(context, "controlnet", subtype="tile", camera_id=None, material_id=self._material_id)
        ...
        for i, camera in enumerate(cameras):
            ...
            if type == 'depth':
                ...
                depth_map_path = get_file_path(context, "controlnet", subtype="tile", camera_id=i, material_id=self._material_id)
    ...
    def generate_flux(self, context, depth_path=None, canny_path=None, normal_path=None, tile_path=None):
    ...
    def refine_flux(self, context, depth_path=None, canny_path=None, normal_path=None, tile_path=None, mask_path=None, render_path=None):
        """     
        ...
        # Set up image inputs for different controlnet types
        self._refine_configure_images_flux(prompt, depth_path, canny_path, normal_path, tile_path, render_path, NODES)
        
        # Build controlnet chain for refinement if needed
        if not context.scene.generation_method == 'uv_inpaint':
            prompt = self._refine_build_controlnet_chain_flux(prompt, context, depth_path, canny_path, normal_path, tile_path, NODES)
    ...
    def _refine_configure_images_flux(self, prompt, depth_path, canny_path, normal_path, tile_path, render_path, NODES):
    ...
    def _refine_build_controlnet_chain_flux(self, prompt, context, depth_path, canny_path, normal_path, tile_path, NODES):
    ...
    # ---------- CHROMA ----------
    def create_base_prompt_chroma(self, context):
        """Creates and configures the base Chroma prompt.
        Uses prompt_text_chroma and does not include negative prompt or LoRA configuration.
        """
        from .util.helpers import prompt_text_chroma
        prompt = json.loads(prompt_text_chroma)
        # Define node IDs for Flux
        NODES = {
            'pos_prompt': "6",          # CLIPTextEncode for positive prompt
            'neg_prompt': "7",          # CLIPTextEncode for negative prompt
            'vae_loader': "10",         # VAELoader
            'dual_clip': "11",          # DualCLIPLoader
            'unet_loader': "12",        # UNETLoader
            'sampler': "13",            # SamplerCustomAdvanced
        ...
        prompt[NODES['scheduler']]["inputs"]["steps"] = context.scene.steps
        prompt[NODES['scheduler']]["inputs"]["scheduler"] = context.scene.scheduler
        prompt[NODES['cfgguider']]["inputs"]["cfg"] = context.scene.cfg
        prompt[NODES['ksampler']]["inputs"]["sampler_name"] = context.scene.sampler
    ...
    def generate_chroma(self, context, depth_path=None, canny_path=None, normal_path=None, tile_path=None):
        """Generates an image using Chroma.
        Similar in structure to generate() but uses Chroma nodes, skips negative prompt and LoRA.
        """
        from .util.helpers import prompt_text_chroma
        server_address = context.preferences.addons[__package__].preferences.server_address
        client_id = str(uuid.uuid4())
        ...
        # Build Chroma base prompt and node mapping.
        prompt, NODES = self.create_base_prompt_chroma(context)
        
        self._configure_resolution(prompt, context, NODES)

        prompt, final_node = self._build_controlnet_chain_extended(
            context, prompt, NODES['pos_prompt'], NODES['neg_prompt'], NODES['vae_loader'],
            {"depth": depth_path, "canny": canny_path, "normal": normal_path, "tile": tile_path}
        )
        # Connect final node to FluxGuidance
        prompt[NODES['flux_guidance']]["inputs"]["conditioning"] = [final_node, 0]
        # Save prompt for debugging.
        try:
            with open(os.path.join(output_dir, "prompt_chroma.json"), 'w') as f:
                json.dump(prompt, f, indent=2)
        except Exception as e:
            print(f"Failed to save flux prompt: {e}")
        # Execute generation via websocket.
        ...
        print(f"Chroma image generated with prompt: {context.scene.comfyui_prompt}")

        return images[NODES['save_image']][0]

    def _create_img2img_base_prompt_chroma(self, context):
        """Creates and configures the base Chroma prompt for img2img refinement."""
        from .util.helpers import prompt_text_img2img_chroma
        
        prompt = json.loads(prompt_text_img2img_chroma)
        
        # Node IDs organized by functional category for Chroma
        NODES = {
            # Text Prompting
            'pos_prompt': "6",          # CLIPTextEncode for positive prompt
            'neg_prompt': "7",          # CLIPTextEncode for negative prompt
            ...
            # Advanced Features
            'differential_diffusion': "50", # DifferentialDiffusion for Chroma
            'inpaint_conditioning': "51",   # InpaintModelConditioning for Chroma
        ...
        # Set positive prompt (Chroma doesn't use negative prompt)
        prompt[NODES['pos_prompt']]["inputs"]["text"] = base_prompt_text
        prompt[NODES['neg_prompt']]["inputs"]["text"] = context.scene.comfyui_negative_prompt
        ...
        prompt[NODES['scheduler']]["inputs"]["steps"] = context.scene.refine_steps if context.scene.generation_method == 'grid' else context.scene.steps
        prompt[NODES['scheduler']]["inputs"]["denoise"] = context.scene.denoise if context.scene.generation_method in ['grid', 'refine'] else 1.0
        prompt[NODES['cfgguider']]["inputs"]["cfg"] = context.scene.refine_cfg if context.scene.generation_method == 'grid' else context.scene.cfg
        prompt[NODES['ksampler']]["inputs"]["sampler_name"] = context.scene.refine_sampler if context.scene.generation_method == 'grid' else context.scene.sampler
        prompt[NODES['scheduler']]["inputs"]["scheduler"] = context.scene.refine_scheduler if context.scene.generation_method == 'grid' else context.scene.scheduler
    ...
    def refine_chroma(self, context, depth_path=None, canny_path=None, normal_path=None, tile_path=None, mask_path=None, render_path=None):
        """     
        Refines the image using Chroma in ComfyUI.         
        :param context: Blender context.         
        :param depth_path: Path to the depth map image.
        :param canny_path: Path to the canny edge image.
        :param mask_path: Path to the mask image for inpainting.
        ...
        # Initialize the img2img prompt template for Chroma
        prompt, NODES = self._create_img2img_base_prompt_chroma(context)
        
        # Configure based on generation method
        self._configure_refinement_mode_chroma(prompt, context, render_path, mask_path, NODES)
        
        # Set up image inputs for different controlnet types
        self._refine_configure_images_chroma(prompt, depth_path, canny_path, normal_path, tile_path, render_path, NODES)
        
        # Build controlnet chain for refinement if needed
        if not context.scene.generation_method == 'uv_inpaint':
            prompt = self._refine_build_controlnet_chain_chroma(prompt, context, depth_path, canny_path, normal_path, tile_path, NODES)
        
        # Save prompt for debugging
        with open(os.path.join(output_dir, "prompt_chroma_img2img.json"), 'w') as f:
            json.dump(prompt, f)
        ...
        print(f"Image refined with Chroma using prompt: {context.scene.comfyui_prompt}")
        
        # Return the refined image
        return images[NODES['save_image']][0]

    def _configure_refinement_mode_chroma(self, prompt, context, render_path, mask_path, NODES):
        """Configures the prompt based on the specific refinement mode for Chroma."""
        # Configure based on generation method
        if context.scene.generation_method == 'refine':
            # Configure for refine mode - load render directly
            prompt[NODES['render_image']]["inputs"]["image"] = render_path
            ...
            if context.scene.differential_diffusion:
                # Connect differential diffusion between model loader and other components
                prompt[NODES['cfgguider']]["inputs"]["model"] = [NODES['differential_diffusion'], 0]
                prompt[NODES['scheduler']]["inputs"]["model"] = [NODES['differential_diffusion'], 0]
            ...
            if context.scene.generation_method == 'uv_inpaint':
                self._configure_uv_inpainting_mode_chroma(prompt, context, render_path, NODES)
            else:  # Sequential mode
                self._configure_sequential_mode_chroma(prompt, context, NODES)

    def _configure_uv_inpainting_mode_chroma(self, prompt, context, render_path, NODES):
        """Configures the prompts for UV inpainting mode in Chroma."""
        # UV inpainting specific configuration
        prompt[NODES['upscale_uv']]["inputs"]["image"] = [NODES['input_image'], 0]
    ...
    def _configure_sequential_mode_chroma(self, prompt, context, NODES):
        """Configures the prompt for sequential generation mode in Chroma."""
        # Direct connection for sequential mode
        if not context.scene.differential_diffusion:
            prompt[NODES['vae_encode_inpaint']]["inputs"]["pixels"] = [NODES['input_image'], 0]
        else:
        ...
        # Note: Chroma doesn't support IPAdapter in the same way as SDXL

    def _refine_configure_images_chroma(self, prompt, depth_path, canny_path, normal_path, tile_path, render_path, NODES):
        """Configures the input images for the refinement process in Chroma."""
        # Set render image if provided
        if render_path:
            prompt[NODES['render_image']]["inputs"]["image"] = render_path
    ...
    def _refine_build_controlnet_chain_chroma(self, prompt, context, depth_path, canny_path, normal_path, tile_path, NODES):
        """Builds the ControlNet chain for refinement process with Chroma."""
        input = NODES['neg_prompt'] if not context.scene.differential_diffusion else NODES['inpaint_conditioning']
        # For Chroma, the controlnet chain connects to the guidance node
        prompt, final_node = self._build_controlnet_chain_extended(
            context, prompt, input, input, NODES['vae_loader'],
            {"depth": depth_path, "canny": canny_path, "normal": normal_path, "tile": tile_path}
        )
