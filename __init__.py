...
def update_combined(self, context): # Combined with load_handler to load controlnet unit on first setup
    ...
    # ControlNet unit reset
    if hasattr(context.scene, 'controlnet_units'):
        controlnet_items = get_controlnet_models(self, context)
        valid_controlnet_ids = {item[0] for item in controlnet_items}

        for id, controlnet_unit in enumerate(context.scene.controlnet_units):
            if controlnet_unit.model_name not in valid_controlnet_ids or controlnet_unit.model_name == "NONE_AVAILABLE":
                # Remove the unit
                context.scene.controlnet_units.remove(id)
    # Check if the current controlnet unit index is valid
    if context.scene.controlnet_units_index >= len(context.scene.controlnet_units) or context.scene.controlnet_units_index < 0:
        context.scene.controlnet_units_index = max(0, len(context.scene.controlnet_units) - 1)
...
class StableGenAddonPreferences(bpy.types.AddonPreferences):
    ...
    external_checkpoints_dir: bpy.props.StringProperty(
        name="External Checkpoints Directory 1",
    ...
    external_checkpoints_dir2: bpy.props.StringProperty(
        name="External Checkpoints Directory 2",
        description="Path to an additional directory for checkpoint models. Ensure ComfyUI is also configured to see this path (e.g., via extra_model_paths.yaml).",
        default="",
        subtype='DIR_PATH',
        update=update_combined,
    ...
    external_controlnet_dir: bpy.props.StringProperty(
        name="External ControlNet Directory (Optional)",
        description="Path to an additional directory for ControlNet models. Ensure ComfyUI is also configured to see this path.",
        default="",
        subtype='DIR_PATH',
        update=update_combined,
    ) # type: ignore    
    ...
    def draw(self, context):
        ...
        split.label(text=self.bl_rna.properties['external_checkpoints_dir2'].name)
        split.prop(self, "external_checkpoints_dir2", text="")
        split = layout.split(factor=0.5)
        ...
        split.label(text=self.bl_rna.properties['external_controlnet_dir'].name)
        split.prop(self, "external_controlnet_dir", text="")
...
def update_model_list(self, context):
    ...
    external_ckpts_dir2 = addon_prefs.external_checkpoints_dir2
    ...
    if comfyui_base_dir and os.path.isdir(comfyui_base_dir):
        ...
        all_model_items.append(
            get_models_from_directory(comfy_ckpts_path, ('.safetensors', '.ckpt', '.pth', '.gguf'), "Checkpoint")
        )

    # 2. Scan External Checkpoints Path
    if external_ckpts_dir and os.path.isdir(external_ckpts_dir):
        all_model_items.append(
            get_models_from_directory(external_ckpts_dir, ('.safetensors', '.ckpt', '.pth', '.gguf'), "Ext. Checkpoint")
        )

    if external_ckpts_dir2 and os.path.isdir(external_ckpts_dir2):
        all_model_items.append(
            get_models_from_directory(external_ckpts_dir2, ('.safetensors', '.ckpt', '.pth', '.gguf'), "Ext. Checkpoint")
        )
...
def get_controlnet_models(context, unit_type):
    addon_prefs = context.preferences.addons[__package__].preferences
    comfyui_base_dir = addon_prefs.comfyui_dir
    external_controlnet_dir = addon_prefs.external_controlnet_dir

    all_controlnet_items = []

    # 1. Scan ComfyUI standard ControlNet path
    if comfyui_base_dir and os.path.isdir(comfyui_base_dir):
        comfy_controlnet_path = os.path.join(comfyui_base_dir, "models", "controlnet")
        all_controlnet_items.append(
            get_models_from_directory(comfy_controlnet_path, ('.safetensors', '.ckpt', '.pt', '.pth'), "LoRA")
        )

    # 2. Scan External LoRAs Path
    if external_controlnet_dir and os.path.isdir(external_controlnet_dir):
        all_controlnet_items.append(
            get_models_from_directory(external_controlnet_dir, ('.safetensors', '.ckpt', '.pt', '.pth'), "Ext. LoRA")
        )
        
    return merge_and_deduplicate_models(all_controlnet_items)
...
class AddControlNetUnit(bpy.types.Operator):
    ...
    unit_type: bpy.props.EnumProperty(
        name="Type",
        items=[('depth', 'Depth', ''), ('canny', 'Canny', ''), ('normal', 'Normal', ''), ('tile', 'Tile', '')],
        default='depth',
        update=update_parameters
    ) # type: ignore
...
def unregister():   
    ...
    del bpy.types.Scene.camera_background
